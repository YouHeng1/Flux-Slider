...
(function($) {
 flux.slider = function(elem, opts) {
  ...
  this.transitions = [];
  for (var fx in flux.transitions) this.transitions.push(fx);
  ...
  $(this.options.transitions).each(function(index, tran) {
   ...
   if (t.options.requires3d && !flux.browser.supports3d) compatible = false;

   if (t.options.compatibilityCheck) compatible = t.options.compatibilityCheck();

   if (compatible) newTrans.push(tran);
  });
  ...
  this.container.bind('click',
      function(event) {
       if ($(event.target).hasClass('hasLink')) window.location = $(event.target).data('href');
      });
 ...
 flux.slider.prototype = {
  ...
  finishedLoading: function() {
   ...
   if (this.options.pagination) {
    ...
    $(this.images).each(function(index, image) {
     ...
     if (index == 0) li.css('margin-left', 0).addClass('current');
    });
  ...
  transition: function(transition, opts) {
   ...
   try {
    tran = new flux.transitions[transition](this, $.extend(this.options[transition] ? this.options[transition] : {},
        opts));
   } catch(e) {
  ...
  updateCaption: function() {
   ...
   if (this.options.captions && this.captionBar) {
    if (str !== "") this.captionBar.html(str);
  ...
  setNextIndex: function(nextIndex) {
   if (nextIndex == undefined) nextIndex = this.currentImageIndex + 1;
 ...
 flux.browser = {
  ...
  supportsCSSProperty: function(prop) {
   ...
   for (var i = 0; i < domPrefixes.length; i++) {
    if (domPrefixes[i] + prop in div.style) support = support || true;
   }
  ...
  rotate: function(axis, deg) {
   ...
   if (flux.browser.supports3d) return 'rotate3d(' + (axis == 'x' ? '1': '0') + ', ' + (axis == 'y' ? '1': '0') + ', ' + (axis == 'z' ? '1': '0') + ', ' + deg + 'deg)';
   else {
    if (axis == 'z') return 'rotate(' + deg + 'deg)';
    else return '';
   }
  }
 };
...
(function($) {
  ...
  for (var i = 0; i < events.length; i++) {
   this.bind(events[i],
       function(event) {
        ...
        for (var j = 0; j < events.length; j++) $(this).unbind(events[j]);
 ...
 flux.transition.prototype = {
  ...
  hasFinished: false,
  // This is a lock to ensure that the fluxTransitionEnd event is only fired once per tansition
  run: function() {
   var _this = this;
   ...
   setTimeout(function() {
        if (_this.options.execute !== undefined) _this.options.execute.call(_this);
       },
       5);
  },
  finished: function() {
  ...
  return new flux.transition_grid(fluxslider, $.extend({
       ...
       renderTile: function(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
        $(elem).css({
         ...
         'background-size':(this.options.columns*colWidth)+'px '+(this.options.rows*rowHeight)+'px'
        }).css3({
  ...
  return new flux.transition(fluxslider, $.extend({
       ...
       setup: function() {
        var w = this.slider.image1.width(),
            ...
            largestLength = Math.sqrt(w * w + h * h),
        // Largest length is the diagonal
        // How many blocks do we need?
            blockCount = Math.ceil(((largestLength - this.options.blockSize) / 2) / this.options.blockSize) + 1,
        // 1 extra to account for the round border
            fragment = document.createDocumentFragment();
        console.log(w)
        console.log(h)
 ...
 flux.transitions.slide = function(fluxslider, opts) {
  return new flux.transition(fluxslider, $.extend({
       ...
       execute: function() {
        ...
        if (this.options.direction == 'left') delta = -delta;
